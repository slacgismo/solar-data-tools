...
class RightMatrixMinimization(AbstractMinimization):
    ...
    def _define_variables_and_parameters(
        self, l_cs_value, r_cs_value, beta_value, component_r0
    ):
        self.left_matrix = cvx.Parameter(
            shape=(self._power_signals_d.shape[0], self._rank_k)
        )
        self.left_matrix.value = l_cs_value
        self.right_matrix = cvx.Variable(
            shape=(self._rank_k, self._power_signals_d.shape[1])
        )
        self.right_matrix.value = r_cs_value
        self.beta = cvx.Variable()
        self.beta.value = beta_value
        self.r0 = cvx.Parameter(len(component_r0))
        self.r0.value = 1.0 / component_r0
        return
    ...
    def _term_f3(self, l_cs_param, r_cs_param):
        ...
        if self._power_signals_d.shape[1] > 365:
            term_f3 = self._mu_r * cvx.norm(
                r_tilde[1:, :-365] - r_tilde[1:, 365:], "fro"
            )
        else:
            term_f3 = self._mu_r * cvx.norm(r_tilde[:, :-365] - r_tilde[:, 365:], "fro")
        return term_f3
    ...
    def _constraints(self, l_cs_param, r_cs_param, beta_param, component_r0):
        ...
        if self._power_signals_d.shape[1] > 365:
            ...
            if self._is_degradation_calculated:
                ...
                constraints.append(
                    cvx.multiply(component_r0[:-365], r[365:] - r[:-365]) == 0
                )
        if self._non_neg_constraints:
            constraints.extend([l_cs_param @ r_cs_param >= 0, r_cs_param[0] >= 0])
        return constraints
    ...
    def _obtain_r_tilde(self, r_cs_param):
        ...
        if r_cs_param.shape[1] < 365 + 2:
            ...
            r_tilde = cvx.hstack(
                [r_cs_param, cvx.Variable(shape=(self._rank_k, n_tilde))]
            )
        else:
            r_tilde = r_cs_param
        return r_tilde
