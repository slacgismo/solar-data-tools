...
class PlotMixin(object):
    def plot_lr(self, figsize=(14, 10), show_days=False):
        ...
        ax[0, 0].legend(["$\\ell_1$"])
        ax[1, 0].legend(
            ["$\\ell_{}$".format(ix) for ix in range(2, self._r_cs_value.shape[0] + 1)]
        )
        ax[0, 1].legend(["$r_{1}$"])
        ax[1, 1].legend(
            ["$r_{}$".format(ix) for ix in range(2, self._r_cs_value.shape[0] + 1)]
        )
        if show_days:
            use_day = self._obtain_weights_for_plotting() > 1e-1
            days = np.arange(self._power_signals_d.shape[1])
            ax[0, 1].scatter(
                days[use_day], self._r_cs_value[0][use_day], color="orange", alpha=0.7
            )
        plt.tight_layout()
        return fig

    def plot_energy(
        self, figsize=(12, 6), show_days=True, show_clear=True, scale_power=False
    ):
        if scale_power:
            c = 1.0 / 1000
        else:
        ...
        plt.plot(
            np.sum(self._power_signals_d, axis=0)
            * 24
            * c
            / self._power_signals_d.shape[0],
            linewidth=1,
            alpha=0.7,
            label="measured daily energy",
        )
        if show_clear:
            plt.plot(
                (self._r_cs_value[0] * np.sum(self._l_cs_value[:, 0]))
                * 24
                * c
                / self._power_signals_d.shape[0],
                linewidth=1,
        ...
        if show_days:
            ...
            plt.scatter(
                days[use_day],
                np.sum(self._power_signals_d, axis=0)[use_day]
                * 24
                * c
                / self._power_signals_d.shape[0],
                color="orange",
                alpha=0.7,
                label="detected clear days",
            )
        plt.legend()
    ...
    def plot_data_matrix(self, figsize=(12, 6), show_days=False, units="kW"):
        if show_days:
            ...
            use_day = None
        fig = plot_2d(
            self._power_signals_d, figsize=figsize, units=units, clear_days=use_day
        )
        return fig

    def plot_measured_clear_matrices(
        self, figsize=(10, 10), show_days=False, units="kW"
    ):
        with sns.axes_style("white"):
            ...
            if show_days:
                ...
                use_day = None
            plot_2d(self._power_signals_d, ax=ax[0], clear_days=use_day, units=units)
            plot_2d(self.clear_sky_signals(), ax=ax[1], clear_days=use_day, units=units)
            ax[0].set_xlabel("")
    ...
    def plot_time_series(
        self, start_day=0, num_days=2, figsize=(8, 4), loc=(0.35, 0.7)
    ):
        d1 = start_day
        d2 = d1 + num_days
    ...
    def plot_time_series_with_weights(
        self, fig_title=None, start_day=0, num_days=5, figsize=(16, 8)
    ):
        n = self._power_signals_d.shape[0]
        d1 = start_day
        d2 = d1 + num_days
        ...
        ax[1].plot(
            xs,
            np.repeat(self._obtain_weights_for_plotting()[d1:d2], n),
            linewidth=1,
            label="day weight",
        )
        ax[0].legend()
