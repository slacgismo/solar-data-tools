...
def load_results():
    ...
    slac_data["all-pass"] = np.logical_and(
        np.alltrue(
            np.logical_not(slac_data[["solver-error", "f1-increase", "obj-increase"]]),
            axis=1,
        ),
...
def load_sys(n=None, idnum=None, local=True, meta=None):
    ...
    if n is not None:
        ...
        return
    df = pd.read_csv(
        base + "PVOutput/{}.csv".format(idnum),
        index_col=0,
        parse_dates=[0],
        usecols=[1, 3],
    )
    tz = meta["TimeZone"][n]
    df.index = df.index.tz_localize(tz).tz_convert(
        "Etc/GMT+{}".format(TZ_LOOKUP[tz])
