...
class PointBrowser(object):
    ...
    def __init__(self, data, xlim=None, ylim=None, prcntl=95):
        ...
        (self.line,) = self.ax[0].plot(
            self.xs, self.ys, ".", picker=5
        )  # 5 points tolerance
        m = np.logical_and(
            np.logical_and(
                self.data["res-median"]
                < np.percentile(self.data["res-median"], prcntl),
        ...
        pts = (min(xlim[0], ylim[0]), max(xlim[1], ylim[1]))
        self.ax[0].plot(pts, pts, ls="--", color="red")
        self.ax[0].set_xlim(xlim)
        self.ax[0].set_ylim(ylim)
        self.text = self.ax[0].text(
            0.05, 0.95, "system ID: none", transform=self.ax[0].transAxes, va="top"
        )
        (self.selected,) = self.ax[0].plot(
            [self.xs[0]],
            [self.ys[0]],
            "o",
            ms=6,
        ...
        with sns.axes_style("white"):
            ax.append(
                plt.axes([0.77, 0.5 * (1 + 0.57), 0.2, 0.05 / 2])
            )  # Text box entry
            ax.append(
                plt.axes([0.82, 0.5 * (1 + 0.5), 0.1, 0.05 / 2])
            )  # run SCSF button
    ...
    def clicked(self, event):
        ...
        if fn in cached_files:
            iterative_fitting = IterativeFitting.load_instance(self.scsf_cache_dir + fn)
            self._iterative_fitting = iterative_fitting
            self.ax[4].plot(
                np.sum(iterative_fitting.power_signals_d, axis=0)
                * 24
            ...
            for cntr in range(CONFIG1["max_iteration"]):
                ...
                improvement = (old_obj - new_obj) * 1.0 / old_obj

                self.ax[3].cla()
                self.ax[3].set_xlabel("Day number")
                self.ax[3].set_yticks([])
                self.ax[3].set_ylabel("(sunset)        Time of day        (sunrise)")
                s1 = "Iteration {} complete: obj = {:.2f}, f1 = {:.2f}".format(
                    cntr + 1, new_obj, obj_vals[0]
                )
                s2 = "Improvement: {:.2f}%".format(100 * improvement)
                tf = time.time()
                s3 = "Time elapsed: {:.2f} minutes".format((tf - ti) / 60.0)
                textout = "\n".join([s1, s2, s3])
        ...
        self.ax[4].plot(
            (
                iterative_fitting.r_cs_value[0]
                * np.sum(iterative_fitting.l_cs_value[:, 0])
            )
            * 24
            / iterative_fitting.power_signals_d.shape[0],
            linewidth=1,
        ...
        with sns.axes_style("white"):
            ...
            bar = self.ax[3].imshow(
                iterative_fitting.clear_sky_signals(),
                cmap="hot",
                vmin=0,
                vmax=np.max(iterative_fitting.power_signals_d),
                interpolation="none",
                aspect="auto",
            )
    ...
    def show_ticks(self, ax):
        ...
        y1 = np.ones_like(days[use_day]) * self._power_signals_d.shape[0] * 0.99
        ax.scatter(days[use_day], y1, marker="|", color="yellow", s=2)
        ax.scatter(days[use_day], 0.995 * y1, marker="|", color="yellow", s=2)
        ax.set_xlim(*xlim)
        ax.set_ylim(*ylim)
        return
    ...
    def update(self):
        ...
        if self.data.iloc[dataind]["res-median"] > np.percentile(
            self.data["res-median"], prcntl
        ):
            l1 = out2.split("\n")
            i = idxs[self.data.columns == "res-median"][0]
            l1[i] = "*" + l1[i][:-2] + "*"
            out2 = "\n".join(l1)
        if self.data.iloc[dataind]["res-var"] > np.percentile(
            self.data["res-var"], prcntl
        ):
            l1 = out2.split("\n")
            i = idxs[self.data.columns == "res-var"][0]
            l1[i] = "*" + l1[i][:-2] + "*"
            out2 = "\n".join(l1)
        if self.data.iloc[dataind]["res-L0norm"] > np.percentile(
            self.data["res-L0norm"], prcntl
        ):
            l1 = out2.split("\n")
        ...
        with sns.axes_style("white"):
            ...
            power_signals_d = (
                df.loc[start:end].iloc[:-1].values.reshape(288, -1, order="F")
            )
            self._power_signals_d = power_signals_d
            self.ax[2].cla()
            foo = self.ax[2].imshow(
                power_signals_d, cmap="hot", interpolation="none", aspect="auto"
            )
...
def view_ts(pb, clear_day_start=None, day_start=None):
    if pb.iterative_fitting is not None:
        clear_days = np.arange(len(pb.iterative_fitting.weights))[
            pb.iterative_fitting.weights >= 1e-3
        ]
        _ = pb.iterative_fitting.ts_plot_with_weights(
            num_days=len(pb.iterative_fitting.weights),
            figsize=(9, 6),
