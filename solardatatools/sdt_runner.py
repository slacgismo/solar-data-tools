...
@click.option('--db_list', help='File containing sites to be retrieved from the cassandra database.')
@click.option('--n_workers', default=1, help='Number of parallel workers to process with.')
@click.option('--n_threads', default=1, help='Number of threads per worker.')
@click.option('--mem_limit', default='16GB', help='Maximum memory that each worker is able to use.')
@click.option('--track_times', default=False, help='Tracks the total time to process each data set.')
@click.option('--worker_log_file', default="", help='Output to save worker logs to.')
@click.argument('output')
def cli(dir, db_list, n_workers, n_threads, mem_limit, track_times, worker_log_file, output):
    ...
    if dir:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            reports = run_pipelines(get_csvs_in_dir(temp_dir), local_csv_to_dh, track_times)
    else:
