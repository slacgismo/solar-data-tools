...
class ClearSkyDetection:
    ...
    def hinge0(self, val, q98):
        return 0.0 if (val <= q98 * self.threshold_low or val >= q98 * self.threshold_high) else 1.0

    def hinge1(self, val, q98):
        return 0.0 if (val >= q98 * self.threshold_low and val <= q98 * self.threshold_high) else 1.0
    ...
    def find_optimal_path(self, L):
        ...
        for t in range(1, self.T):
            cum_loss[0, t] += min(cum_loss[1, t - 1] + self.stickiness_high, cum_loss[0, t - 1])
            cum_loss[1, t] += min(cum_loss[0, t - 1] + self.stickiness_low, cum_loss[1, t - 1])
