...
class PolarTransform:
    ...
    def normalize_days(self):
        ...
        quantiles = series.groupby(series.index.date).aggregate(
            lambda x: np.nanquantile(x, 0.95)
        )
        normed_data = pd.Series(
            index=series.index, dtype=float, name="normalized-power"
        )
        for ix in quantiles.index:
            day = ix.strftime("%x")
            if quantiles.loc[ix] >= 0:
                power_normed = series.loc[day] / quantiles.loc[ix]
            else:
                power_normed = np.zeros_like(series.loc[day])
