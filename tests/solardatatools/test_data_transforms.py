...
class TestStandardizeTimeAxis(unittest.TestCase):
    ...
    def test_standardize_time_axis(self):
        data_file_path = pathlib.Path(__file__).parent.parent.joinpath("fixtures/data_transforms/timeseries.csv")
        data = pd.read_csv(data_file_path, index_col=0, parse_dates=True)
        expected_data_file_path = pathlib.Path(__file__).parent.parent.joinpath("fixtures/data_transforms/timeseries_standardized.csv")
...
class TestMake2D(unittest.TestCase):
    ...
    def test_make_2d_with_freq_set(self):
        data_file_path = pathlib.Path(__file__).parent.parent.joinpath("fixtures/data_transforms/timeseries_standardized.csv")
        data = pd.read_csv(data_file_path, index_col=0, parse_dates=True)
        expected_data_file_path = pathlib.Path(__file__).parent.parent.joinpath("fixtures/data_transforms/power_mat.csv")
