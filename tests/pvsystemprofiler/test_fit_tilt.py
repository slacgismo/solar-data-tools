...
class TestFitTilt(unittest.TestCase):
    def test_fit_tilt(self):
        ...
        # costheta_fit
        filepath = Path(__file__).parent.parent
        costheta_fit_file_path = (
            filepath / "fixtures" / "tilt_azimuth" / "costheta_fit.csv"
        )
        with open(costheta_fit_file_path) as file:
            costheta_fit = np.genfromtxt(file, delimiter=",")
        # boolean_filter
        filepath = Path(__file__).parent.parent
        boolean_filter_file_path = (
            filepath / "fixtures" / "tilt_azimuth" / "boolean_filter.csv"
        )
