...
from pvsystemprofiler.scripts.modules.script_functions import (
    run_failsafe_lon_estimation,
)
from pvsystemprofiler.scripts.modules.script_functions import (
    run_failsafe_lat_estimation,
)
...
def evaluate_systems(site_id, inputs_dict, df, site_metadata, json_file_dict=None):
    partial_df_cols = [
        ...
        "data_sampling",
        "data quality_score",
        "data clearness_score",
        "inverter_clipping",
        "time_shifts_corrected",
    ...
    if json_file_dict is not None:
        partial_df_cols.extend(
            ["zip_code", "real longitude", "real latitude", "real tilt", "real azimuth"]
        )
    ...
    for col_label in cols:
        if col_label.find(inputs_dict["power_column_label"]) != -1:
            ...
            if system_id in site_metadata["system"].tolist():
                ...
                if inputs_dict["time_shift_manual"]:
                    time_shift_manual = int(
                        site_metadata.loc[sys_mask, "time_shift_manual"].values[0]
                    )
                ...
                dh, passes_pipeline = run_failsafe_pipeline(
                    dh,
                    sys_tag,
                ...
                if passes_pipeline:
                    results_list = [
                        ...
                        passes_pipeline,
                        dh.num_days,
                        dh.capacity_estimate,
                        dh.data_sampling,
                        dh.data_quality_score,
                        dh.data_clearness_score,
                        dh.inverter_clipping,
                        dh.time_shifts,
                        dh.tz_correction,
                        dh.capacity_changes,
                    ...
                    if json_file_dict is not None:
                        if system_id in json_file_dict.keys():
                            ...
                            json_information = extract_sys_parameters(
                                source_file, system_id, inputs_dict["s3_location"]
                            )
                        else:
                    ...
                    results_list = [site_id, system_id, passes_pipeline] + [np.nan] * (
                        len(results_list) - 3
                    )
                ...
                if inputs_dict["estimation"] == "longitude":
                    ...
                    results_df, passes_estimation = run_failsafe_lon_estimation(
                    ...
                    results_df, passes_estimation = run_failsafe_lat_estimation(
                        dh, real_latitude
                    )
                    ...
                    if inputs_dict["estimated_longitude"]:
                        longitude_input = float(
                            site_metadata.loc[sys_mask, "estimated_longitude"]
                        )
                    ...
                    if inputs_dict["gmt_offset"]:
                        ...
                        gmt_offset = float(site_metadata.loc[sys_mask, "gmt_offset"])
                    results_df, passes_estimation = run_failsafe_ta_estimation(
                        dh,
                        1,
                        None,
                        longitude_input,
...
def main(full_df, inputs_dict, df_system_metadata):
    ...
    for file_ix, file_id in enumerate(file_list):
        t0 = time()
        msg = "Site/Accum. run time: {0:2.2f} s/{1:2.2f} m".format(
            site_run_time, total_time / 60.0
        )
        ...
        if inputs_dict["file_label"] is not None:
            ...
            site_id = file_id[:i]
            mask = (
                df_system_metadata["site"]
                == site_id.split(inputs_dict["file_label"])[0]
            )
        else:
        ...
        if inputs_dict["data_source"] == "s3":
            df = load_generic_data(
                inputs_dict["s3_location"], inputs_dict["file_label"], site_id
            )
        if inputs_dict["data_source"] == "cassandra":
            df = load_cassandra_data(site_id)

        if not site_metadata.empty:
            partial_df = evaluate_systems(
                site_id, inputs_dict, df, site_metadata, json_file_dict
            )
        else:
            partial_df = None
...
if __name__ == "__main__":
    ...
    if ssf is not None:
        df_system_metadata = load_system_metadata(
            df_in=ssf, file_label=inputs_dict["file_label"]
        )
