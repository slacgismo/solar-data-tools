class ConfigPartitions:
    def __init__(
        ...
        n_files=None,
        file_label=None,
        fix_time_shifts=None,
        time_zone_correction=None,
        check_json=None,
        sup_file=None,
        ds=None,
        gmt=None,
        ...
        if ix_0 is not None and ix_n is not None:
            ...
            self.local_working_folder = (
                self.local_working_folder_location
                + "estimation_part_{}_of_{}/".format(str(part_id + 1), str(n_part))
            )
            self.local_input_file = (
                self.local_working_folder
                + "input_part_{}_of_{}.csv".format(str(part_id + 1), str(n_part))
            )
            self.local_output_file_name = "results_part_{}_of_{}.csv".format(
                str(part_id + 1), str(n_part)
            )
            self.local_output_file = (
                self.local_working_folder + self.local_output_file_name
            )
            self.ix_0 = ix_0
            self.ix_n = ix_n
...
def get_config(
    ...
    if ix_0 is not None and ix_n is not None:
        ...
        return ConfigPartitions(
            est=est, ifl=ifl, skf=skf, au=au, ain=ain, ar=ar, ac=ac, gof=gof, god=god
        )
